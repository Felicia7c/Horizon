...
手机 RSS 阅读器建议订阅逐条精选流：[Horizon Selected RSS]({{ '/feeds/horizon-selected-zh.xml' | relative_url }})。
...
For phone RSS readers, subscribe to the item-level feed: [Horizon Selected RSS]({{ '/feeds/horizon-selected-zh.xml' | relative_url }}).
